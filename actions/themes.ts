...
async function getCurrentUserId(): Promise<string> {
  // Temporairement retourner un ID fictif pour éviter la dépendance auth
  return "temp-user-id";
  /*
...
// Layer 1: Clean server actions with proper error handling - TEMPORAIREMENT SIMPLIFIÉ
export async function getThemes() {
  try {
    // Temporairement retourner des thèmes fictifs
    const mockThemes = [
      {
        id: "theme-1",
        userId: "temp-user-id",
...
export const getTheme = cache(async (themeId: string) => {
  try {
    ...
    // Temporairement retourner un thème fictif
    if (themeId === "theme-1") {
      return {
        id: "theme-1",
        userId: "temp-user-id",
...
export async function createTheme(formData: {
  ...
  try {
    ...
    // Temporairement simplifié - pas de limite de thèmes
    const { name, styles } = validation.data;
    const newThemeId = cuid();
    const now = new Date();

    // Retourner un thème fictif créé
    const mockTheme = {
      id: newThemeId,
      userId: userId,
      name: name,
...
export async function updateTheme(formData: {
  ...
  try {
    ...
    // Temporairement retourner un thème mis à jour fictif
    const mockTheme = {
      id: themeId,
      userId: userId,
      name: name || "Thème mis à jour",
      styles:
        styles ||
        ({
          light: { primary: "hsl(222.2 47.4% 11.2%)" },
...
export async function deleteTheme(themeId: string) {
  try {
    ...
    // Temporairement retourner un résultat fictif de suppression
    if (themeId === "theme-1") {
      return { id: themeId, name: "Thème supprimé" };
    }
