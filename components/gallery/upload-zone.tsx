...
import { X, Trash2, Upload, Image, AlertCircle, FileText, Archive } from "lucide-react";
...
const fileRejectionMessages = {
  ...
  "file-too-many-documents": "Vous ne pouvez uploader que 10 documents à la fois",
...
export const UploadZone = ({ children }: UploadZoneProps) => {
  ...
  const validateFile = (file: File): string | null => {
    ...
    if (!imageTypes.includes(file.type) && !documentTypes.includes(file.type) && !archiveTypes.includes(file.type)) {
    ...
    const currentImagesCount = currentFiles.filter(f => imageTypes.includes(f.type)).length;
    const currentDocumentsCount = currentFiles.filter(f => documentTypes.includes(f.type)).length;
    const currentArchivesCount = currentFiles.filter(f => archiveTypes.includes(f.type)).length;

    if (imageTypes.includes(file.type) && currentImagesCount >= config.limits.maxFilesPerType.images) {
      return `Vous ne pouvez pas uploader plus de ${config.limits.maxFilesPerType.images} images à la fois`;
    }
    if (documentTypes.includes(file.type) && currentDocumentsCount >= config.limits.maxFilesPerType.documents) {
      return `Vous ne pouvez pas uploader plus de ${config.limits.maxFilesPerType.documents} documents à la fois`;
    }
    if (archiveTypes.includes(file.type) && currentArchivesCount >= config.limits.maxFilesPerType.archives) {
  ...
  const getFileIcon = (file: FileWithPreview) => {
    ...
    if (file.type.includes('pdf') || file.type.includes('word') || file.type.includes('text')) return <FileText className="w-4 h-4" />;
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDropRejected(fileRejections, event) {
      for (const file of fileRejections) {
        toast.error(`${file.file.name}: ${fileRejectionMessages?.[file.errors[0]?.code as keyof typeof fileRejectionMessages] || "Erreur inconnue"}`);
      }
    },
    accept: Object.keys(acceptedFileTypes).length > 0 ? acceptedFileTypes : undefined,
    noClick: true,
      ...
      <Button
        ...
        onClick={(e) => {
          ...
          input.accept = Object.entries(acceptedFileTypes)
            .map(([type, exts]) => [...(type === 'image/*' ? [type] : []), ...exts])
            .join(',');
          input.onchange = (e) => {
        ...
        <div className="absolute inset-0 bg-primary/10 backdrop-blur-sm flex items-center justify-center z-50">
          <div className="bg-background/95 p-8 rounded-lg shadow-lg text-center border-2 border-dashed border-primary">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              ...
              <br />
              Taille maximale: {config.maxFileSize}MB
            </p>
          </div>
        </div>
      )}
        ...
        <div className="fixed top-0 right-0 bottom-0 w-96 bg-background border-l shadow-lg z-50 flex flex-col">
          <div className="p-4 border-b">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-lg font-semibold">
                Fichiers à uploader ({filesToUpload.filter(f => !f.error).length}/{filesToUpload.length})
              </h3>
              <Button
                variant="ghost"
                size="icon"
            ...
            <div className="flex gap-2">
              ...
              <Button
                size="sm"
                onClick={uploadFiles}
                disabled={isUploading || filesToUpload.filter(f => !f.error).length === 0}
                className="flex-1"
              >
                <Upload className="w-4 h-4 mr-2" />
          ...
          <div className="p-4 border-t mt-auto">
            <Button
              ...
              disabled={isUploading || Object.keys(acceptedFileTypes).length === 0}
            >
              <Upload className="w-4 h-4 mr-2" />
      ...
      {Object.keys(acceptedFileTypes).length === 0 && (
        <div className="fixed bottom-6 right-6 z-40">
          <Alert variant="destructive" className="w-96">
            ...
            <AlertDescription>
              L'upload de fichiers est actuellement désactivé. Veuillez activer au moins un type de fichier dans la configuration.
            </AlertDescription>
          </Alert>
        </div>
      )}
