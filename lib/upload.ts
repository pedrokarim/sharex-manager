...
async function getThumbnailPath(
  ...
  const thumbnailPath = join(uploadPath, config.storage.thumbnailsPath);
...
async function saveFile(
  ...
  await chmod(filePath, Number.parseInt(permissions, 8));
}
  ...
  try {
    ...
    if (/\.(jpg|jpeg|png|gif|webp)$/i.test(file.name)) {
      try {
        ...
        console.error(
          "Erreur lors du traitement de l'image par les modules:",
          error
        );
